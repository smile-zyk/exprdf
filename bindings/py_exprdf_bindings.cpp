...
py::object invoke_df_unary(const char* op_name, const exprdf::DataFrame& self) {
    py::tuple args(1);
    args[0] = py::cast(self);
    return exprdf::pyops::invoke(op_name, args);
}

py::object invoke_df_binary(const char* op_name, const exprdf::DataFrame& self, py::handle rhs) {
    py::tuple args(2);
    args[0] = py::cast(self);
    args[1] = py::reinterpret_borrow<py::object>(rhs);
    return exprdf::pyops::invoke(op_name, args);
}
} // namespace

void bind_types_and_dataframe(py::module_& m) {
    ...
    py::class_<exprdf::DataFrame, std::shared_ptr<exprdf::DataFrame>>(m, "DataFrame")
        ...
        // ----------------------------------------------------------------
        .def("__add__", [](const exprdf::DataFrame& self, py::object rhs) {
            return invoke_df_binary("add", self, rhs);
        }, py::arg("rhs"), "Element-wise add on last column (rhs: DataFrame/scalar/1-D numpy array)")
        .def("__sub__", [](const exprdf::DataFrame& self, py::object rhs) {
            return invoke_df_binary("sub", self, rhs);
        }, py::arg("rhs"), "Element-wise subtract on last column (rhs: DataFrame/scalar/1-D numpy array)")
        .def("__mul__", [](const exprdf::DataFrame& self, py::object rhs) {
            return invoke_df_binary("mul", self, rhs);
        }, py::arg("rhs"), "Element-wise multiply on last column (rhs: DataFrame/scalar/1-D numpy array)")
        ...
        .def("__rsub__", [](const exprdf::DataFrame& self, py::object lhs) {
            return invoke_df_binary("rsub", self, lhs);
        }, py::arg("lhs"), "Element-wise reverse subtract on last column (lhs: DataFrame/scalar/1-D numpy array)")
        .def("__rmul__", [](const exprdf::DataFrame& self, py::object lhs) {
            return invoke_df_binary("rmul", self, lhs);
        }, py::arg("lhs"), "Element-wise reverse multiply on last column (lhs: DataFrame/scalar/1-D numpy array)")
        ...
        // conj / max / min as methods
        .def("conj", [](const exprdf::DataFrame& self) {
            py::tuple args(1);
            args[0] = py::cast(self);
            return exprdf::pyops::invoke("conj", args);
        },
